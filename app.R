...
ui <- dashboardPage(
    ...
    dashboardSidebar(
        sidebarMenu(
            ...
            selectInput("country", "Choose country", choices = unique(data$country), 
...
server <- function(input, output) {
    ...
    output$plot5 <- renderPlot({
        my.data.frame1_geo %>% 
            group_by(admin2) %>% 
            summarise(N_events = sum(n)) %>%  ggplot +
    ...
    output$evol <- renderPlot({
        data %>% filter(country %in% input$country) %>%
            ...
            xlab("Evolution (September 2019 - September 2022)")
